...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const startDateStr = new Date(dateStart);
  const endDateStr = new Date(dateEnd);

  const diffInMs = endDateStr - startDateStr;
...
function isDateInPeriod(date, period) {
  ...
  const normalizedStart = normalize(startPeriod);
  const normalizedEnd = normalize(endPeriod);
